...
for s in range(3):
	...
	if args.ml:
		print(f'{FC}Polynomials: {pol_count[s]}{FR}')
	...
	if args.sat:
		...
		if args.guess:
		    ...
		    ## default branch
		    if args.sep:
		        ml_gb = ps.magma_ml(firstl_txt[s],pil_txt[s],lastl_txt[s])
		        print_time(ps.time_log(), 'magma_ml')
		    else:
		        ml_gb=[]
		    keys = ps.keys(ml_gb, firstl_txt[s],pil_txt[s],lastl_txt[s], meth='magma', ml=args.ml, sep=args.sep)
