...
class AES(sr.SR_gf2, metaclass=Timer):
    ...
    def ps_reduce_ml(self,ps,meth='PAM',num=1, psets=1):
        ...
        if meth == "PAM":
            dists = np.empty(shape=[num*bits,bits*psets], dtype=int)
            meds=sample(rng,bits*psets)
            for p in rng:
                for i in range(bits*psets):
            ...
            for i in range(bits*psets):
                ...
                if len(pidxs[0]) == 1:
                    ...
                    polys.append(allpolys[pidxs[0][0]])
                elif len(pidxs[0]) == 2:
                    ##cluster has 2 polynomials - take xor
                    polys.append(allpolys[pidxs[0][0]] + allpolys[pidxs[0][1]])
                else:
                    ...
                    for i in range(len(pidxs[0])):
                        ...
                        while j < len(pidxs[0]):
                            ...
                            j+=1
                    ##xor of best pair
                    polys.append(allpolys[cl_idx[0]] + allpolys[cl_idx[1]])
                ...
                if polysxorflags[i]:
                    ...
                    while len(polysxorlm[i]) == len(polysxorlm[j]):
                        if polysxorlm[i] == polysxorlm[j]:
                            polysxorflags[j]=False
                            if polysxorflags[i]:
                                polysxorflags[i]=False
                                polys.append(allpolys[polysxoridx[i]] + allpolys[polysxoridx[j]])
                                count += 1
                                if count >= bits*psets:
                                    flagend=True
                                break
            ...
            ##needed from 3.rund
            while count < bits*psets:
                polys.append(allpolys[count])
                count+=1
            ...
            while count<bits*psets:
                if sorted_candidates[i][0] == 0:
                    i+=1
                    continue
                polys.append(allpolys[sorted_candidates[i][1]] + allpolys[sorted_candidates[i][2]])
                count+=1
                i+=1
        ...
        mmax=10000
        pil_txt=[]
        lastl_txt="gb := GroebnerBasis(Ideal(["
        for i in range(len(polys)):
            ...
            pil_txt.append(pil_txt_arr)

        lastl_txt=lastl_txt.rstrip(", ")
        lastl_txt+="]): Faugere, Boolean, HFE);\n\ngb;\n"
        firstl_txt="P<"
        kv_txt=f'{polys[0].ring().gens()}'
        kv_txt=kv_txt.strip("()")
        firstl_txt+=f'{kv_txt}> := BooleanPolynomialRing({bits}, "grevlex");\n\n'

        with open('../out/magma.txt', 'w') as f:
            f.write(f"{firstl_txt}")
        with open('../out/magma.txt', 'a') as f:
            for pol_arr in pil_txt:
                for pol in pol_arr:
                    f.write(f"{pol}")
            f.write(f"{lastl_txt}")
        ...
        return firstl_txt,pil_txt,lastl_txt,len(polys), round(mean([len(p) for p in polys])), round(mean([m.deg() for p in polys for m in p.monomials()]))
